...
from recbole_pjf.sampler import *
...
def create_samplers_for_multi_direction(config, dataset, built_datasets):
    ...
    if train_neg_sample_args['distribution'] != 'none':
        if not config['repeatable']:
            sampler = PJFSampler(phases, built_datasets, train_neg_sample_args['distribution'])
        else:
            sampler = PJFSampler(phases, dataset, train_neg_sample_args['distribution'])
        train_sampler = sampler.set_phase('train')

    if eval_neg_sample_args['distribution'] != 'none':
        if g_sampler is None or j_sampler is None:
            if not config['repeatable']:
                g_sampler = PJFSampler(phases, built_datasets, eval_neg_sample_args['distribution'])
                built_datasets[0].change_direction()
                j_sampler = PJFSampler(phases, built_datasets, eval_neg_sample_args['distribution'])
            else:
                g_sampler = PJFSampler(phases, dataset, eval_neg_sample_args['distribution'])
                built_datasets[0].change_direction()
                j_sampler = PJFSampler(phases, dataset, eval_neg_sample_args['distribution'])
        else:
