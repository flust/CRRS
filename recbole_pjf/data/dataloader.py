...
class PJFTrainDataLoader(TrainDataLoader):
    def __init__(self, config, dataset, sampler, shuffle=False):
        ...
        if config['pos_sample']:
            self._sampler.pos_used_ids = self._sampler.get_pos_used_ids(1)
            self.used_ids_pos_i = self._sampler.pos_used_ids

        self.change_field_direction()
        self._sampler.used_ids = self._sampler.get_used_ids()

        if config['pos_sample']:
            self._sampler.pos_used_ids = self._sampler.get_pos_used_ids(2)
            self.used_ids_pos_u = self._sampler.pos_used_ids
        ...
        if config['pos_sample']:
            self._sampler.pos_used_ids = self.used_ids_pos_i
    ...
    def _pos_sampling(self, inter_feat):
        user_ids = inter_feat[self.uid_field].numpy()
        item_ids = inter_feat[self.iid_field].numpy()
        pos_item_ids = self._sampler.pos_sample_by_user_ids(
            user_ids, item_ids, self.neg_sample_num
        )
        inter_feat = inter_feat.repeat(self.times)
        pos_item_feat = Interaction({self.iid_field: pos_item_ids})
        pos_item_feat = self._dataset.join(pos_item_feat)
        pos_item_feat.add_prefix('pos_')
        inter_feat.update(pos_item_feat)
        return inter_feat

    def _neg_sampling(self, inter_feat):
        inter_feat_neg_i = super(PJFTrainDataLoader, self)._neg_sampling(inter_feat)
        if self.config['pos_sample']:
            inter_feat_pos_i = self._pos_sampling(inter_feat)
        ...
        if self.config['pos_sample']:
            self._sampler.pos_used_ids = self.used_ids_pos_u
            inter_feat_pos_u = self._pos_sampling(inter_feat)
        ...
        if self.config['pos_sample'] == True:
            inter_feat.update(inter_feat_pos_i)
            inter_feat.update(inter_feat_pos_u)
        return inter_feat
